...
try:
    ...
    for item in items[2:]:
        ...
        if (int(death) > int(confirmed)) or (int(recovered) > int(confirmed)):
            potential_error.append(
                (name + " Confirmed: " + confirmed + ", Recovered: " + recovered + ", Death: " + death))
    ...
    with open("assets/virus.csv", "w", encoding="utf-8") as fp:
        fp.write(
            str(col_name_list)[1:len(str(col_name_list)) - 1].replace("\'", "").replace(", ", ",").replace("(null)",
                                                                                                           "") + "\n")
        for row in cursor.execute("SELECT * from virus"):
            line = str(row)[1:len(str(row)) - 1].replace("\'", "").replace("None", "").replace(", ", ",").replace(
                "(null)",
                "") + "\n"
            flag = line[0:10]
        ...
        for row in cursor.execute("SELECT * from virus order by rowid DESC limit 1"):
            ...
            if confirmed != priorConfirmed:
                fp.write(
                    str(row)[1:len(str(row)) - 1].replace("\'", "").replace("None", "").replace(", ", ",").replace(
                        "(null)",
                        "") + "\n")
    ...
    password = open("steven_gmail_password", "r").read().split("\n")[2]
    message = """\
    Subject: Update Error - COVID-19 Website

    The auto update script encountered a problem while updating. Please check.
